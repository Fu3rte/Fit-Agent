...
from app.api.schemas.analytics_dto import (
    MonthQuery,
    calendar_dto,
    personal_best_dto,
    trends_dto,
...
async def list_personal_bests(request: Request) -> dict[str, Any]:
    ...
    bests = await app_services(request).stats.list_personal_bests()
...
async def get_trends(
    ...
    report = await app_services(request).stats.trends(business_day)
...
async def get_calendar(request: Request, month: MonthQuery) -> dict[str, Any]:
    ...
    calendar = await app_services(request).stats.calendar_month(year, month_number)
