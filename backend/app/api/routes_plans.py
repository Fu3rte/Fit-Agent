...
from app.api.schemas.plan_dto import (
    UnknownResource,
    plan_dto,
    plan_session_dto,
)
...
async def list_plans(request: Request) -> dict[str, Any]:
    ...
    plans = await app_services(request).plans.list_versions()
...
async def get_active_plan(request: Request) -> dict[str, Any]:
    ...
    plan = await app_services(request).plans.read_active()
...
async def get_plan(plan_id: int, request: Request) -> dict[str, Any]:
    ...
    plan = await app_services(request).plans.read_by_id(plan_id)
    if plan is None:
...
async def list_plan_sessions(plan_id: int, request: Request) -> dict[str, Any]:
    ...
    plans = app_services(request).plans
    if await plans.read_by_id(plan_id) is None:
        raise UnknownResource(f"计划不存在：{plan_id}")
    sessions = await plans.list_sessions(plan_id)
