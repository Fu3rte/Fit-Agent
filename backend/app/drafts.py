...
class DraftService:
    ...
    async def revise_profile_draft(
        ...
        - **只接受档案草稿允许纠错的内容**（stage2.md §5 S2-04 拍板 2B：普通档案字段、
          动作限制、身体情况均允许内联纠错）：拟议内容整体替换，并做与创建路径同口径
          的结构校验与限制引用校验；不做首次建档完整性判定（完整性是确认入口契约，
          归 S2-05）。允许用户纠错不等于 Agent 获得自动解除限制或改写正式身体情况的
          权限——本方法只改 Pending 草稿，正式限制与身体情况的阻断判定只在确认事务按
          最终内容复查后生效。
