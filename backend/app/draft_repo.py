...
async def _select_row(
    ...
    async with conn.execute(
        "SELECT id, conversation_id, run_id, base_profile_json, proposed_profile_json,"
        " base_business_version, revision, status, committed_revision,"
        " committed_business_version, created_at, updated_at"
...
class DraftRepo:
    ...
    async def create_pending(
        ...
        *,
        ...
        """保存一条 Pending 草稿（revision 从 1 起）并返回落库后的行。
        ...
        （stage2.md §4.1）。本方法不写正式档案、不推进 ``context_version``、不校验领域规则。
    ...
    async def list_for_conversation(self, conversation_id: str) -> tuple[Draft, ...]:
        """该会话已持久化草稿的当前状态（不依赖历史通知，01 1.2）。"""

        async def op(conn: aiosqlite.Connection) -> tuple[Draft, ...]:
            async with conn.execute(
                "SELECT id, conversation_id, run_id, base_profile_json,"
