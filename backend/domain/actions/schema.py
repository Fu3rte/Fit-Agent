...
class Exercise:
    ...
    #: 主要肌群（03 3.1 已拍补充）：exercises-dataset 的 ``target`` 原样落库；未收录为 None。
    #: 上游 ``muscle_group``／``secondary_muscles`` 是协力肌来源，不混入本值。
    ...
    def from_row(cls, row: Mapping[str, Any]) -> "Exercise":
        ...
        return cls(
            ...
            instructions_zh=(
                None if row["instructions_zh"] is None else str(row["instructions_zh"])
            ),
            # 空串与 NULL 同等视为未收录：缺失值不得被读成某个肌群（替换等价 fail-closed）。
            muscle=None if muscle is None or not str(muscle).strip() else str(muscle),
        )
