...
async def test_simulated_system_tz_change_keeps_saved_timezone_and_business_date(
        ...
        instant = datetime(
                2026, 6, 1, 16, 30, tzinfo=UTC
        )  # 上海 6/2 00:30；纽约 6/1 12:30
...
async def test_dst_region_rules_not_fixed_offset(tmp_path: Path) -> None:
        ...
        cases = [
                ...
                (
                        TZ_NEW_YORK,
                        datetime(
                                2026, 7, 1, 4, 0, tzinfo=UTC
                        ),  # EDT -4: 6/30 24:00 → 7/1 00:00
                        date(2026, 7, 1),
        ...
        for case in cases:
                ...
                assert business_date(summer_instant, saved) == summer_date, (
                        timezone_name
                )
                assert business_date(winter_instant, saved) == winter_date, (
                        timezone_name
                )
