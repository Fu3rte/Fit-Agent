...
# 依据：本轮拍板的 FitnessIntent Schema（domain／action／execution_type 三字段，日期槽已移除）、
# 唯一纯映射函数 FitnessIntent → Intent、取消确定性短语旁路。
...
from app.application.ports import dump_model_payload
...
LEGAL_ROUTES: tuple[tuple[RouteKey, Intent], ...] = (
    ...
    (
        RouteKey("workout_execution", "create", "natural_language_record"),
...
def test_every_literal_combination_either_maps_or_is_rejected() -> None:
    ...
    for domain, action, execution_type in product(
        get_args(Domain),
        get_args(Action),
        ...
        try:
            ...
            continue
        key = RouteKey(domain, action, execution_type)
        mapped.add(key)
...
#: 三个字段的 Schema 描述必须覆盖的语义片段：字段名 → 描述须包含的关键词。
...
def test_router_prompt_requires_the_execution_type_null_placeholder() -> None:
    """提示词写明 strict Schema 只有三个字段，未用的 execution_type 必须显式输出 null。"""
    assert (
        "strict Schema 只含 domain、action、execution_type 三个字段："
        "未用到的 execution_type 必须在输出里显式给出 null，不得省略。"
