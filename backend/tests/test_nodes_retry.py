...
# 非瞬时失败立即上抛且只尝试一次。依据：本轮批准的重试口径——429／500／502／503／504、SDK 的连接／
# 读超时异常与显式过载失败可重试；取消、401／403、配置、Schema 与预算耗尽一律不重试。
...
def _wrapped_failure(cause: Exception) -> ModelCallFailed:
    """按生产 lazy 入口的方式包装：``ModelCallFailed`` 的显式 cause 是 Provider 的原始失败。"""
    try:
...
def _status_failure(error_type: type[Any], status_code: int) -> Exception:
    """构造带真实 Provider 状态的 SDK 错误：类型与状态码共同决定是否可重试。"""
    return error_type(
        "Provider 瞬时故障",
...
#: 可重试的瞬时失败：连接／读超时异常与 429／500／502／503／504／529。
...
#: 不可重试的失败：鉴权、配额、Schema、预算与取消都只尝试一次。
...
async def test_lazy_gateway_wrapper_cause_is_unwrapped_for_classification(
    sleeps: list[float],
) -> None:
    """生产 lazy 入口把调用失败统一包成 ``ModelCallFailed``：重试按 ``__cause__`` 识别瞬时失败。"""
    model = ScriptedModel([
        _wrapped_failure(anthropic.APITimeoutError(_REQUEST)),
        ANSWER,
    ])

    assert await request_model(model.gateway(), "系统提示", {}, ModelRequestBudget()) == ANSWER
...
async def test_structured_request_shares_the_same_retry(
    ...
    model = ScriptedModel(
        [_status_failure(anthropic.ServiceUnavailableError, 503), expected]
    )
    budget = ModelRequestBudget()
...
async def test_retry_is_refused_when_the_request_budget_is_exhausted(
    ...
    model = ScriptedModel([openai.APITimeoutError(_REQUEST), ANSWER])
