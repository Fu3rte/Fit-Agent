"""计划与计划日程只读路由（Stage 1 子任务 04 §9）：无任何写入入口。

传输边界：只调用 ``domain.plans.service.PlanReadService``（只读用例），不做领域规则、不写 SQL；
计划创建、确认与激活留到阶段 5。路由顺序：``/api/plans/active`` 必须声明在
``/api/plans/{plan_id}`` 之前，否则会被路径参数吞掉。
"""
...
async def list_plans(request: Request) -> dict[str, Any]:
    ...
    plans = await PlanReadService(request.app.state.db).list_versions()
...
async def get_active_plan(request: Request) -> dict[str, Any]:
    """当前 active 计划；没有正式启用的计划时发 ``plan: null``（不拿 draft 当替代）。"""
    plan = await PlanReadService(request.app.state.db).get_active()
...
async def get_plan(plan_id: int, request: Request) -> dict[str, Any]:
    ...
    plan = await PlanReadService(request.app.state.db).get_by_id(plan_id)
    if plan is None:
...
async def list_plan_sessions(plan_id: int, request: Request) -> dict[str, Any]:
    ...
    if await service.get_by_id(plan_id) is None:
        raise UnknownResource(f"计划不存在：{plan_id}")
    sessions = await service.list_sessions(plan_id)
