...
> 2. **浏览器 UI / 人工走查**：owner 已于 **2026-09-10** 在 mock 上人工走查 §7 第 1–10 步与 §3.2 U1–U8（**口述确认，未附截图/录屏**）；本文件不因此声称真实链路通过。
> 3. mock 通过 ≠ 真实链路通过：本阶段不接真实后端（无真实 HTTP/SSE/模型调用），未验证项见第 4 节末尾。
> 4. 2026-09-10 据走查结果勾选 `plans/stage1.md` §9 完成条件；未改任何决策正本。
...
| 代码版本（HEAD） | `6b4aee3a1b955915d86016390fa1ead83275a514`（短号 `6b4aee3`，`frontend: Stage 1 建档闭环…`）；前端 Stage 1 变更已入库，协议级证据对应此提交 |
| 工作树状态 | 前端已提交（`6b4aee3`）；§3.2 owner 走查记录与 `plans/stage1.md` §9 勾选为 2026-09-10 增补（同次提交）。仓库仍有非前端改动：`backend/domain/actions/schema.py`、`pre-prj/stage/stage0.md`（已修改）、`pre-prj/stage/stage2.md`（未跟踪） |
| 证据日期 | 协议级 2026-09-09；owner 浏览器人工走查 2026-09-10（本机时间 +08:00） |
...
### 3.2 owner 浏览器人工走查（2026-09-10 执行）

证据强度声明：下表为 **owner 口述确认**（会话回复「能看到，没问题」，针对本节列出的走查清单），**未附截图/录屏**；本阶段不建浏览器测试框架（存档 B6），故无法自动复核。协议级对应断言见 §2（同分支 HTTP 行为已过）。

| # | 走查项 | 依据 | 结论 |
|---|---|---|---|
| U1 | `/profile` 未建档引导卡渲染 +「前往对话开始建档」跳转；对话页未建档引导文案渲染 | stage1 §5 F1-04、§7 第 1 步 | 通过 |
| U2 | 设置页录入 Key 的 UI 流程 → 回对话页可输入 | §7 第 2 步 | 通过 |
| U3 | 档案草稿卡六类字段/体重/限制/红旗渲染、Diff 呈现、必含元素齐全 | §5 F1-03、§7 第 5 步 | 通过 |
| U4 | 草稿卡内联纠错控件实际交互（频率/体重数字、器械与限制增删、粒度）→ `revision+1`、Diff 实时更新 | §5 F1-03、§7 第 6 步 | 通过 |
| U5 | 确认采纳 toast + 看板失效刷新；`/profile` 档案卡 + 限制卡红色「暂禁」徽章 +「尚无计划」占位 | §7 第 7 步 | 通过 |
| U6 | 草稿待确认时刷新 → 草稿卡按查询恢复；确认后刷新 → `committed` 态与档案页一致 | §7 第 9 步 | 通过 |
| U7 | 45s 无事件 → EventSource 自动关闭转 `GET /api/runs/active` 轮询 | stage0 F0-04、§7 第 9 步 | 通过（含在 owner 口述确认内，未单独复述） |
| U8 | 断线/刷新恢复、无重复拼接、无通知重放；幂等重复确认、丢弃后拒绝确认、stale → 一键重算 → 再次确认 | stage0 §7 第 5 步、stage1 §7 第 8/10 步 | 通过 |
...
|---|---|---|---|---|---|---|
| F1-01 契约微调与空种子 | 静态 + mock 进程 | `6b4aee3` | `npm run build`；探针 A1–A10 | `tsc -b` 零错误；空种子五页数据源可查询、未建档态为 `profile=null`、`context_version=0` | 一致（构建 EXIT=0；空种子断言全过） | 本文 §1、§2 |
...
| F1-03 档案草稿卡与确认 | mock 协议级 + owner 浏览器（2026-09-10） | `6b4aee3` | 探针 F、R、C、I、ST、RC、CB、DC；§3.2 U3–U5 | 确认前正式档案不变；纠错 revision+1 且 Diff 重派生；确认写入档案+限制、`context_version+1`；幂等原结果；stale 拦截、重算新草稿、丢弃拒绝；草稿卡渲染与内联控件可用 | 协议分支一致；草稿卡渲染/内联纠错/确认 toast/档案页由 owner 走查确认通过 | 本文 §2、§3.2 |
| F1-04 未建档空态与档案页更新 | 代码级 + owner 浏览器（2026-09-10） | 同上 | 探针 A6–A8、C5–C11；§3.2 U1/U5 | 未建档态可查询；建档后 `/api/profile` 返回档案+限制；空态引导与建档后展示正确、页面无编辑入口 | 数据源一致；未建档引导/跳转、档案卡、红色「暂禁」限制卡、「尚无计划」占位由 owner 走查确认通过 | 本文 §2、§3.2 |
| F1-05 闭环演示与证据 | 构建 + mock HTTP 探针 + owner 浏览器走查 | 同上 | `npm run build`；`f1-05-probe.sh`；§7 第 1–10 步人工走查 | 第 7 节剧本从空种子完整走通、构建零错误、证据完整 | 构建 EXIT=0；协议级 110/110 + 30/30；owner 2026-09-10 浏览器完整走查确认通过 → 结项门槛达成 | 本文全文、§3.2 |

## 5. 风险与观察（交评审）

1. **结项门槛已达成（2026-09-10）**：§3.2 U1–U8 由 owner 在浏览器人工走查确认通过；证据强度为 **owner 口述确认（无截图/录屏）**，协议级对应断言见 §2。
2. **证据版本**：代码证据对应 `6b4aee3`（前端 Stage 1 已提交）；§3.2 走查记录与 `plans/stage1.md` §9 勾选为 2026-09-10 增补。
