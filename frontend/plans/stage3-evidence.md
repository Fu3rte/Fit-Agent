...
> 对应 `plans/stage3.md` 第 5 节 F3-01–06 与第 6 节证据格式。协议级闭环已完成；**owner 浏览器主路径（约 §7.1–9）已走查并修复问题；§7.10–14 仍待 owner 复测**。
> **证据分层：**
> 1. **协议级**（mock HTTP / 模块级）：§1–§3 实际执行结果，可复现。
> 2. **UI 渲染 / 浏览器走查**：主路径已走查（无截图/录屏，口头验收）；10–14 未完成。
...
| F3-06 | §7 协议级串联与缺口；build 零错误；证据归档；mock 边界写明 | 协议级 PASS；owner 主路径已走查，**10–14 待复测** |
...
- **owner 浏览器 §7.10–14**（无安排加练、同日多练、待补全、安全/stale/丢弃、刷新恢复 UI 路径）
...
Owner 已走查主路径（约 §7.1–9）并驱动修正（目标用力文案、参考重量、罗马尼亚匹配、辅助词表、记录侧 RIR 隐藏）。  

**仍缺**：owner 浏览器 §7.10–14 复测；完成后可勾 stage3.md §9 第 2 条并标 Stage 3 全量结项。mock 通过不得声称为真实链路通过；F1/F5–F9 与安排读回端点仍留联调阶段。
