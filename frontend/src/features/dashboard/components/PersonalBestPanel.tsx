...
import { ChevronLeft, ChevronRight, Search, Star, Trophy } from "lucide-react";
...
/** 三类 PB 落到的展示维度：配色与图标口径来自 DIMENSION_STYLE 的唯一出处 */
const PB_DIMENSION: Record<PersonalBestTypeWire, DimensionKey> = {
  weight_pb: "weight",
...
const DIMENSION_TABS: Array<{ key: DimensionFilter; label: string }> = [
  { key: "all", label: "全部" },
  ...(Object.keys(PB_DIMENSION) as PersonalBestTypeWire[]).map((key) => ({
    key,
    label: DIMENSION_STYLE[PB_DIMENSION[key]].label,
  })),
];
...
export default function PersonalBestPanel({
  ...
  const [pinnedKeys, setPinnedKeys] = useState<string[]>(() =>
    (localStorage.getItem(PIN_STORAGE_KEY) ?? "").split(",").filter(Boolean),
  );
  ...
  const togglePin = (key: string) => {
    const next = pinnedKeys.includes(key)
      ...
      : [...pinnedKeys, key];
    setPinnedKeys(next);
    localStorage.setItem(PIN_STORAGE_KEY, next.join(","));
  };
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="flex flex-1 flex-col gap-4">
        <section className="flex items-center gap-2">
          ...
          >
            {pinned.map((pb) => {
              const style = DIMENSION_STYLE[PB_DIMENSION[pb.pb_type]];
              return (
                <div
                  key={pbKey(pb)}
                  className={`flex w-40 shrink-0 flex-col justify-between gap-2 rounded-xl border bg-linear-to-b from-card to-muted/20 p-3 ${style.border}`}
                >
        ...
        <section className="flex flex-col gap-2 border-t pt-3">
          ...
          <div
            className={`flex min-h-74 flex-col gap-1 ${
              filtered.length === 0 ? "justify-center" : ""
            }`}
          >
            {pageItems.map((pb) => {
              const style = DIMENSION_STYLE[PB_DIMENSION[pb.pb_type]];
