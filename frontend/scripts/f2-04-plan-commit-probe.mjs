/**
 * F2-04 验收探针（走真实 mock REST）：计划启用、替换与原子日程切换
 ...
 *      （档案补丁 + 新版本 + 新日程 + 旧版未来未锁定日程取消）、已锁定日程不动、
 *      故障注入整份回滚（档案／计划与历史／全部日程／草稿状态／context_version）、
 *      draft_modified 与 draft_stale→重算、丢弃后拒绝确认、「今天只能用哑铃」不持久化。
 * 不覆盖：草稿卡浏览器渲染与 /profile 计划卡（F2-05/F2-06）、真实后端计划域事务与
 *        真实数据库日程锁定、复核日后的续期与自动跨周期（本阶段不做）。
 */
...
/** 发起一次对话，不等结果；返回 run_id（供流式期间的并发断言） */
...
/** 轮询等 Run 进入终态，返回该 Run 关联的最后一张草稿 */
...
/**
 * 等流式输出真正开始（已发出第一个 message.delta）：此时提案已在生成处派生完毕，
 * 而草稿尚未登记——正是「流式期间被其他确认推进 context_version」的窗口。
 */
async function waitStreaming(runId) {
    for (let i = 0; i < 120; i += 1) {
    ...
    throw new Error("Run 未在预期时间内开始流式输出");
}
...
const scheduleCount = (status, entryStatus, planVersion) =>
    status.schedules.filter(
        (s) =>
            s.status === entryStatus &&
...
check(
    "步骤 5 生成结构化计划草稿（新建 v1、12 个应训练日、带候选与取消清单为空）",
    plan1?.kind === "plan_adjust" &&
...
edited.plan.blocks[0].exercises[0].sets = 5;
...
await reset("default");
// 默认种子未配置模型（has_api_key 徒章位）：按设置页链路置位后才可发起对话
...
check(
    "步骤 7 组合草稿：档案器械补丁 + v3 提议 + 12 个新日程 + 旧版未来未锁定日程取消清单（不含已锁定）",
    comp?.kind === "plan_adjust" &&
        comp.status === "pending" &&
        comp.payload?.profile_patch?.profile?.equipment?.join("、") ===
            "哑铃" &&
        comp.payload?.plan?.version === "v3" &&
        comp.payload?.schedules?.length === 12 &&
        (comp.payload?.cancellations ?? []).length === futureBefore &&
        (comp.payload?.cancellations ?? []).every(
...
check(
    "步骤 9 替换提案为结构化载荷（版本只追加）",
    draftA?.kind === "plan_adjust" &&
...
status = await devStatus();
check(
    "步骤 9 先提交另一草案：v4 启用、context_version 推进",
...
/* ---------- 01 §1.3：base_business_version 绑定生成时版本，不回填流式结束后的最新版本 ---------- */
await reset("noplan");
const earlyProposal = (await run("s1", "帮我生成一份训练计划")).draft;
const streamingRun = await startRun("s1", "帮我生成一份训练计划");
await waitStreaming(streamingRun);
// 流式期间另一张草稿被确认（推进 context_version）；待确认草稿的生成已完成，尚属未登记状态
