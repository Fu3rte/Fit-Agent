...
| PR 现算 | `backend/domain/stats/service.py`、`repo.py`、迁移 `010/012` | 复用“从有效记录现算”的思想；重写为四类 PB；旧完成率查询不复用 |
...
| 旧联调脚本 | `backend/scripts/` | 绑定旧 Runtime/PydanticAI 的脚本删除；只按新版最小真实模型案例重建需要的脚本 |
...
- `workout_sets` 记录动作、负重口径、重量、次数和组类型；
...
热身、辅助完成和不完整记录必须在统一入口排除，避免四类 PB 各写一套过滤条件。

四类 PB：

| 类型 | 计算口径 |
| --- | --- |
| `weight_pb` | 同动作、同负重口径的单组最大重量 |
| `reps_pb` | 同动作、同负重口径、同重量的单组最大次数 |
| `estimated_1rm_pb` | 仅外加重量且 1–10 次，`重量 × (1 + 次数 / 30)` |
| `volume_pb` | 同动作单次训练内有效工作组容量最大值 |

每个结果返回来源训练、来源组和发生时间。外加重量与自重分组，不混算。PB 不建结果表。
...
`trend_summary` 不包含计划完成率。

同一底层查询同时服务看板和 MemoryAssembler。每个指标用固定数据库样本测试可复算；没有数据时返回明确空值，不补 0、不让模型估算。
...
2. **数据看板（新增，替换旧 ReviewPage 的统计职责）**：展示月历、单次计划日程状态、体重趋势、体脂趋势、力量/容量趋势和 PB；不展示完成率；引入 Recharts。
...
1. 模型提取日期、动作、负重口径、重量、次数、组数和候选计划日程；
2. Pydantic 校验结构；
...
Gate：固定输入可完成训练和身体数据 CRUD；非法日期、重量、次数、组数和缺字段被拒绝；全仓搜索无 RIR 业务字段残留。
...
- 实现统一有效工作组查询；
- 实现四类 PB 和来源信息；
- 实现日历需要的单次计划日程状态，不实现完成率；
- 实现不含完成率的 `trend_summary`；
- 新增日历及 Recharts 趋势图。
...
在确定性测试通过后，用 OpenAI 兼容端点执行少量固定案例：
...
真实模型测试只验证结构化输出和业务不变量，不恢复旧版费用账本或正式评测平台。
...
- [ ] 表单可新增、修改和删除训练/身体数据；
- [ ] 自然语言打卡先回显日期、动作、重量、次数、组数和关联日程，确认前不写入；
- [ ] 月历、体重、体脂、力量/容量趋势和 PB 可演示；
- [ ] 修改或删除训练记录后，PB 与趋势查询立即一致；热身组、辅助组不得刷新 PB；
- [ ] 无有效训练历史或无可回退负荷时，计划不得生成具体重量；
- [ ] 同一负荷连续两次完成全部目标组且达到次数上限后，才能按 `min_load_increment` 递增；未满足条件不得加重；连续两次未达到次数下限时按规则回退；
- [ ] 额外训练不关联计划日程；同一计划日程最多完成一次；系统不计算或展示完成率；
- [ ] 计划可以生成、评估、至多修订一次、确认或拒绝；
- [ ] 数据库任意时刻最多一条 active 计划；
- [ ] 后续调整读取当前计划和最新数据，不脱离旧计划重新生成；
...
- [ ] `trend_summary` 的体重/体脂变化、训练容量变化和距上次训练天数均可由固定数据库输入复算，且不包含完成率；
- [ ] 启动时只注入 Skill 的 `name` 和 `description`，任务命中后才加载正文；
- [ ] PB、趋势和单次计划日程状态全部由确定性代码计算，LLM 只解释结果；
- [ ] 当前请求精确命中 §6.6 任一关键词时不得进入 Planner；画像中的禁用动作 ID 不得出现在计划中；
- [ ] Checkpoint 恢复与数据库 draft 兜底统一进入同一幂等提交服务，不得双路径提交；
- [ ] API Key 只来自环境变量且不回显、不记录；缺失时不阻塞非模型业务启动；
- [ ] 每个 Graph Run 有请求次数和超时上限，但无费用账本；
- [ ] 全仓无 PydanticAI 与 RIR 运行时代码；
- [ ] 后端测试、lint 和前端 build 通过。
...
- [ ] 当前位于基线 commit 创建的 `refactor/langgraph` 分支；
- [ ] 旧版测试结果已留档；
- [ ] 新数据库文件名和环境变量名已写入 README；
- [ ] `001_initial.sql` 只包含总结要求的业务表；
- [ ] 第一阶段测试能够在临时数据库独立运行。
