...
> 2026-09-12 设计变更（待实现）：努力程度改为仅处方字段（默认 2，保守调整可提高到 3；不要求用户报告、不进入结构化记录、不参与组级判定）；本文件下方与 RIR 相关的实现描述为变更前状态，决策与回填见 `pre-prj/design-decisions.md`（「训练域设计增量」与「Stage 1–3 已拍决策回填」）。
